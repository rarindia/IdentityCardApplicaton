...
function initAvatarCrop(img) {
    img.Jcrop({
        ...
        jcrop_api.setOptions({ aspectRatio: 3/2 });
...
function saveAvatar() {
   ...
   // $('#avatar-crop-box button').addClass('disabled');
    ...
    $.ajax({
        type: "POST",
        url: "/StudentIdentityCard/SaveSignature",
        traditional: true,
        data: {
            w: img.css('width'),
            h: img.css('height'),
        ...
        if (data.success === true) {
            $('#SignImageUpload img').attr('src', data.avatarFileLocation);
            //hidden (.val(data.ArticleFileName))
            $('#SignImage').val((data.ArticleFileName))
            $.magnificPopup.close();
